...
Submission-oriented outputs generated by `run.sh`:
- `workspace/results.log`
- `workspace/result.log`
- `output3.txt`

`output3.txt` is intended to be a reasoning-style submission artifact summarizing:
- run log from the current execution
- runtime design
- optimization path
- validation status
- current benchmark result captured on the execution environment when available
- It is generated as the primary aggregated artifact for the run.
